...
export default function QueueLivePage() {
  ...
  const waitingItems = useMemo(() => items.filter((item) => item.status !== "done"), [items]);
  const current = waitingItems[0];
  const nextUp = waitingItems.slice(1, 7);
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(244,167,185,0.2),_transparent_35%),linear-gradient(180deg,#fffafc_0%,#fff7fb_50%,#fff4f9_100%)] px-3 py-4 text-[#3D2C35] sm:px-4 sm:py-8">
      <div className="mx-auto w-full max-w-6xl space-y-6">
        <header className="rounded-3xl border border-[#f3d8e6] bg-white/80 p-4 shadow-[0_16px_34px_rgba(244,167,185,0.18)] backdrop-blur sm:p-6">
          <p className="text-sm font-semibold text-[#8b6f7a]">LeoKanun Queue Board</p>
          <h1 className="mt-1 text-2xl font-bold sm:text-4xl">🎀 หน้าจอคิวลูกค้า</h1>
          <p className="mt-2 text-sm text-[#7f6771]">เช็กคิวล่าสุดแบบเรียลไทม์ สบายตา อ่านง่าย พร้อมอัปเดตอัตโนมัติทุก 10 วินาที</p>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.5fr,1fr]">
          <article className="rounded-3xl border border-[#f3d8e6] bg-white p-4 shadow-[0_14px_28px_rgba(61,44,53,0.08)] sm:p-6">
            <p className="text-sm font-semibold text-[#8b6f7a]">คิวที่กำลังเรียก</p>
            {loading ? (
          ...
          <article className="rounded-3xl border border-[#f3d8e6] bg-white p-4 shadow-[0_14px_28px_rgba(61,44,53,0.08)] sm:p-6">
            <p className="text-sm font-semibold text-[#8b6f7a]">คิวรอต่อไป</p>
        ...
        <section className="rounded-3xl border border-[#f3d8e6] bg-white/90 p-4 shadow-[0_14px_24px_rgba(61,44,53,0.06)]">
          <p className="text-center text-sm text-[#8b6f7a]">🫶 ขอบคุณที่ใช้บริการ หากต้องการสอบถามเพิ่มเติม แจ้งพนักงานที่เคาน์เตอร์ได้เลย</p>
        </section>
      </div>
    </main>
  );
